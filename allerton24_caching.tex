  ...
  \begin{frame}{Two important distributions:}
	...
	\begin{itemize}
		\item \alert{Inter-arrival distribution:} Typical distance between points...
		 		\begin{equation*}
					\tau_{k+1}-\tau_k \sim F(t), \quad E[\tau_{k+1}-\tau_k] = 1/\lambda 
				\end{equation*} 
		\pause
...
\begin{frame}{Timer based pre-fetching}
	...
	\begin{columns}[T]
		...
		\begin{tikzpicture}[scale=1]
		  ...
		  \draw [->,thick,black] (1.1,.2) -- (3.9,.2) node [above, midway] {$\sim F$};
		...
		\begin{center}
		 ...
		 \shade[left color=azulcito!10!white, right color=white] (5,.9) rectangle (6,0);
		  ...
		  \draw [<-,thick, black] (1.1,.2) -- (4.5,.2) node [above, midway] {$\sim\hat{F}$};
